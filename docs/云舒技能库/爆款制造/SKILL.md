...
*   **解法**: 
    ...
    *   **风格化 BGM**: 自动下载 Lo-Fi/Tech House 风格背景乐，营造沉浸式学习氛围。

### 1.5 视觉增强 (Visual Enhancement)
*   **痛点**: 
    *   手机观看录屏时字体太小。
    *   字幕在复杂背景下看不清。
*   **解法**: 
    ...
    *   **高亮字幕**: 
        *   **关键词**: 自动识别关键词（如 "Trae", "AI"）并渲染为**金黄色 (#FFD700)**。
...
*   **解法**: 
    *   **平滑转场**: 视频片段间自动应用 **FadeIn (淡入)** 效果，消除跳跃感。
    *   **自动封面**: 使用 PIL 自动生成黑金风格封面图，包含标题与 Logo。
    *   **自我校验**: 内置 `validate_video` 模块，在输出前自动检查文件大小与流完整性。
...
/
├── src/                    # 核心代码
│   ├── viral_video_engine.py   # 主程序 (v6.0: 包含 Zoom, OCR, Subtitles)
│   ├── download_music.py       # BGM 下载工具
...
│   └── ffmpeg.exe              # 本地 FFmpeg (可选)
├── output/                 # 产出物
│   ├── final_product_v6.mp4    # 最终成品
...
*   **调整 BGM**: 修改 `src/download_music.py`。
